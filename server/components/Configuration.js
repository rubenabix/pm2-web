...
Configuration.prototype._normaliseHosts = function () {
    ...
    var userDetails = passwdUser.sync(process.getuid());
    ...
    args.forEach(function (host) {
        ...
        if (host.rpc.substring(0, 1) == "~") {
            host.rpc = userDetails.homedir + host.rpc.substring(1)
        }

        if (host.events.substring(0, 1) == "~") {
            host.events = userDetails.homedir + host.events.substring(1)
        }
